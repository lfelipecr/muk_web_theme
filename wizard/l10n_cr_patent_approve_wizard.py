...
class PatentApproveWizard(models.TransientModel):
    ...
    def _get_year_month(self):
        ...
        elif not patent_id.pay_to and patent_id.date_approved:
            year = patent_id.date_approved.year
            month = patent_id.date_approved.month


        #elif not patent_id.date_approved and patent_id.state in ('in_progress'):
    ...
    def _get_percentage_by_trimester(self, fecha):
        ...
        monthRange = calendar.monthrange(year, last_month)
        first_day = date(year, last_month - 2, 1)
        last_day = date(year, last_month, monthRange[1])
    ...
    def evalue_month_selected(self, mod):
        #Año y mes que debería pagar
        year, month = self._get_year_month()
        trimester_selected = trimester_to_number[self.trimester] #trimestrre seleccionado por el usuario.
        trimester_number = trimester_to_number[month_to_trimester[str(month)]] #Trimestre a pagar
        if trimester_selected < trimester_number:
           return -1
        else:
            return 1
    ...
    def _get_sale_lines(self):
        ...
        patent_line_timbre_licor = self.patent_id.patents_lines.filtered(lambda l: l.product_id.id == product_timbre_licor_id and l.active == True)
        if self.patent_id.type_id.code == 'LC':
            if not patent_line_timbre_licor:
                lines.append({
                    'product_id': product_timbre_licor_id,
                    'price_unit': timbre_licor,
                    'year': trimesters[0]['year']
                })
                paid_timbre_licor = True
            elif patent_line_timbre_licor:
                if int(patent_line_timbre_licor[0].year) < int(trimesters[0]['year']):
                    lines.append({
                        'product_id': product_timbre_licor_id,
                        'price_unit': timbre_licor,
                        'year': trimesters[0]['year']
    ...
    def _get_trimesters(self):
        ...
        ultimo_trimestre_pagado = False
        ultimo_anio_pagado = False
        if trimestre_paids:
            t_init = 1
            ultimo_pago = max(trimestre_paids)
            ultimo_anio_pagado = ultimo_pago[0]
            ultimo_trimestre_pagado = ultimo_pago[1]
        elif not trimestre_paids:
            if actual_trimester_number <= trimestre_de_aprobacion:
                t_init = 1
            else:
        ...
        for t in range(t_init, t_last+1):
            ...
            year = datetime.today().year #Año actual

            if ultimo_trimestre_pagado and ultimo_anio_pagado:
                if ultimo_anio_pagado < year and ultimo_trimestre_pagado < 4:
                    if ultimo_trimestre_pagado >= t:
                        not_save = True
                        year = ultimo_anio_pagado
                    else:
                        year = ultimo_anio_pagado
                elif ultimo_anio_pagado == year:
                    year = year
                    if ultimo_trimestre_pagado >= t:
                        not_save = True
            elif not(ultimo_anio_pagado and ultimo_anio_pagado) and fecha_aprobacion:
                mes = fecha_aprobacion.month
                t_aprobacion =trimester_to_number[month_to_trimester[str(mes)]]
                anio = fecha_aprobacion.year
                if t>=t_aprobacion:
                    not_save = False
                    year = anio
                else:
            ...
            if not not_save:
                if t < trimestre_de_aprobacion: #trimestres antes del trimestre de aprobacion
                    not_save = False
                elif t == trimestre_de_aprobacion and year == fecha_aprobacion.year: #si es igual al trimestre de aprobacion, sacar porcentaje
                ...
                if not not_save:
                    ...
                    data = {
                        ...
                        "advance": t > actual_trimester_number and year == datetime.today().year,
        ...
        for inv in self.patent_id.invoices_ids:
            if inv.state == 'posted' and not (inv.payment_state == False or inv.payment_state == 'not_paid') and not inv.reversal_move_id and not inv.reversed_entry_id:
                for line in inv.invoice_line_ids:
                    #if line.year == int(inv.company_id.period): #solo correspondientes a este año
                    if line.trimestre > 0:
                        trimester = line.trimestre
                        trimestre_array.append([date.today().year - 1 if line.year == 0 else line.year,trimester])
                    else:
